...
function buildRotationMetaDebug(prefab, pRot) {
    const rotatedDx = pRot % 2 === 1 ? prefab.dz : prefab.dx;
    const rotatedDz = pRot % 2 === 1 ? prefab.dx : prefab.dz;
    ...
    return { prefabName: prefab && prefab.dx ? 'VilaDeLuxo' : 'unknown', pRot, rotatedDx, rotatedDz, componentCount, groundComponentCount: groundedComponents.size, supportOffsets, componentByKey, blocksRot };
}

const meta = buildRotationMetaDebug(VilaDeLuxo, 0);
console.log(JSON.stringify(meta, null, 2));

// map block -> components
const blockToComponents = VilaDeLuxo.blocks.map((b, i) => {
    const rb = rotatePrefabBlock(VilaDeLuxo, b, 0);
    if (!rb) return { i, comps: [] };
    const comps = new Set();
    for (let x = 0; x < rb.dx; x++) for (let y = 0; y < rb.dy; y++) for (let z = 0; z < rb.dz; z++) {
        const key = `${rb.lx + x},${rb.ly + y},${rb.lz + z}`;
        const comp = meta.componentByKey ? meta.componentByKey.get(key) : null;
        if (comp !== undefined) comps.add(comp);
    }
    return { i, type: b.type, comps: Array.from(comps).sort() };
});

console.log('Block -> components (non-empty only):');
blockToComponents.filter(b => b.comps.length > 0).forEach(b => console.log(b));
