...
function buildRotationMetaForDebug(prefab, pRot) {
    const occupied = [];
    ...
    const groundCells = occupied.filter(c => c.y === 0);
    const groundComponents = new Set(groundCells.map(c => componentByKey.get(`${c.x},${c.y},${c.z}`)));

    return { componentCount, groundComponentCount: groundComponents.size, supportOffsets, groundCellsCount: groundCells.length, groundComponents: Array.from(groundComponents), componentByKey };
}

const meta = buildRotationMetaForDebug(VilaDeLuxo, 0);
console.log('componentCount:', meta.componentCount);
console.log('groundComponentCount:', meta.groundComponentCount);
console.log('groundCellsCount:', meta.groundCellsCount);
console.log('groundComponents (ids):', meta.groundComponents);
console.log('supportOffsets count:', meta.supportOffsets.length);

// list components that have no ground cells
const compHasGround = new Map(meta.componentByKey ? [] : []);
// compute components present
const comps = new Set();
for (const v of meta.componentByKey.values()) comps.add(v);
for (const c of comps) {
    const hasGround = meta.groundComponents.includes(c);
    console.log(`component ${c} hasGround: ${hasGround}`);
}
