export const hiragana = Object.freeze([
    {
        id: 1,
        term: "あ",
        definition: "a",
        syllableGroup: "a",
    },
    {
        id: 2,
        term: "い",
        definition: "i",
        syllableGroup: "i",
    },
    {
        id: 3,
        term: "う",
        definition: "u",
        syllableGroup: "u",
    },
    {
        id: 4,
        term: "え",
        definition: "e",
        syllableGroup: "e",
    },
    {
        id: 5,
        term: "お",
        definition: "o",
        syllableGroup: "o",
    },
    {
        id: 6,
        term: "か",
        definition: "ka",
        syllableGroup: "a",
    },
    {
        id: 7,
        term: "き",
        definition: "ki",
        syllableGroup: "i",
    },
    {
        id: 8,
        term: "く",
        definition: "ku",
        syllableGroup: "u",
    },
    {
        id: 9,
        term: "け",
        definition: "ke",
        syllableGroup: "e",
    },
    {
        id: 10,
        term: "こ",
        definition: "ko",
        syllableGroup: "o",
    },
    {
        id: 11,
        term: "さ",
        definition: "sa",
        syllableGroup: "a",
    },
    {
        id: 12,
        term: "し",
        definition: "shi",
        syllableGroup: "hi",
    },
    {
        id: 13,
        term: "す",
        definition: "su",
        syllableGroup: "u",
    },
    {
        id: 14,
        term: "せ",
        definition: "se",
        syllableGroup: "e",
    },
    {
        id: 15,
        term: "そ",
        definition: "so",
        syllableGroup: "o",
    },
    {
        id: 16,
        term: "た",
        definition: "ta",
        syllableGroup: "a",
    },
    {
        id: 17,
        term: "ち",
        definition: "chi",
        syllableGroup: "hi",
    },
    {
        id: 18,
        term: "つ",
        definition: "tsu",
        syllableGroup: "su",
    },
    {
        id: 19,
        term: "て",
        definition: "te",
        syllableGroup: "e",
    },
    {
        id: 20,
        term: "と",
        definition: "to",
        syllableGroup: "o",
    },
    {
        id: 21,
        term: "な",
        definition: "na",
        syllableGroup: "a",
    },
    {
        id: 22,
        term: "に",
        definition: "ni",
        syllableGroup: "i",
    },
    {
        id: 23,
        term: "ぬ",
        definition: "nu",
        syllableGroup: "u",
    },
    {
        id: 24,
        term: "ね",
        definition: "ne",
        syllableGroup: "e",
    },
    {
        id: 25,
        term: "の",
        definition: "no",
        syllableGroup: "o",
    },
    {
        id: 26,
        term: "は",
        definition: "ha",
        syllableGroup: "a",
    },
    {
        id: 27,
        term: "ひ",
        definition: "hi",
        syllableGroup: "i",
    },
    {
        id: 28,
        term: "ふ",
        definition: "fu",
        syllableGroup: "u",
    },
    {
        id: 29,
        term: "へ",
        definition: "he",
        syllableGroup: "e",
    },
    {
        id: 30,
        term: "ほ",
        definition: "ho",
        syllableGroup: "o",
    },
    {
        id: 31,
        term: "ま",
        definition: "ma",
        syllableGroup: "a",
    },
    {
        id: 32,
        term: "み",
        definition: "mi",
        syllableGroup: "i",
    },
    {
        id: 33,
        term: "む",
        definition: "mu",
        syllableGroup: "u",
    },
    {
        id: 34,
        term: "め",
        definition: "me",
        syllableGroup: "e",
    },
    {
        id: 35,
        term: "も",
        definition: "mo",
        syllableGroup: "o",
    },
    {
        id: 36,
        term: "や",
        definition: "ya",
        syllableGroup: "a",
    },
    {
        id: 37,
        term: "ゆ",
        definition: "yu",
        syllableGroup: "u",
    },
    {
        id: 38,
        term: "よ",
        definition: "yo",
        syllableGroup: "o",
    },
    {
        id: 39,
        term: "ら",
        definition: "ra",
        syllableGroup: "a",
    },
    {
        id: 40,
        term: "り",
        definition: "ri",
        syllableGroup: "i",
    },
    {
        id: 41,
        term: "る",
        definition: "ru",
        syllableGroup: "u",
    },
    {
        id: 42,
        term: "れ",
        definition: "re",
        syllableGroup: "e",
    },
    {
        id: 43,
        term: "ろ",
        definition: "ro",
        syllableGroup: "o",
    },
    {
        id: 44,
        term: "わ",
        definition: "wa",
        syllableGroup: "a",
    },
    {
        id: 45,
        term: "を",
        definition: "wo",
        syllableGroup: "o",
    },
    {
        id: 46,
        term: "ん",
        definition: "n",
        syllableGroup: "n",
    },
    {
        id: 47,
        term: "が",
        definition: "ga",
        syllableGroup: "a",
    },
    {
        id: 48,
        term: "ぎ",
        definition: "gi",
        syllableGroup: "i",
    },
    {
        id: 49,
        term: "ぐ",
        definition: "gu",
        syllableGroup: "u",
    },
    {
        id: 50,
        term: "げ",
        definition: "ge",
        syllableGroup: "e",
    },
    {
        id: 51,
        term: "ご",
        definition: "go",
        syllableGroup: "o",
    },
    {
        id: 52,
        term: "ざ",
        definition: "za",
        syllableGroup: "a",
    },
    {
        id: 53,
        term: "じ",
        definition: "ji",
        syllableGroup: "i",
    },
    {
        id: 54,
        term: "ず",
        definition: "zu",
        syllableGroup: "u",
    },
    {
        id: 55,
        term: "ぜ",
        definition: "ze",
        syllableGroup: "e",
    },
    {
        id: 56,
        term: "ぞ",
        definition: "zo",
        syllableGroup: "o",
    },
    {
        id: 57,
        term: "だ",
        definition: "da",
        syllableGroup: "a",
    },
    {
        id: 58,
        term: "ぢ",
        definition: "ji",
        syllableGroup: "i",
    },
    {
        id: 59,
        term: "づ",
        definition: "zu",
        syllableGroup: "u",
    },
    {
        id: 60,
        term: "で",
        definition: "de",
        syllableGroup: "e",
    },
    {
        id: 61,
        term: "ど",
        definition: "do",
        syllableGroup: "o",
    },
    {
        id: 62,
        term: "ば",
        definition: "ba",
        syllableGroup: "a",
    },
    {
        id: 63,
        term: "び",
        definition: "bi",
        syllableGroup: "i",
    },
    {
        id: 64,
        term: "ぶ",
        definition: "bu",
        syllableGroup: "u",
    },
    {
        id: 65,
        term: "べ",
        definition: "be",
        syllableGroup: "e",
    },
    {
        id: 66,
        term: "ぼ",
        definition: "bo",
        syllableGroup: "o",
    },
    {
        id: 67,
        term: "ぱ",
        definition: "pa",
        syllableGroup: "a",
    },
    {
        id: 68,
        term: "ぴ",
        definition: "pi",
        syllableGroup: "i",
    },
    {
        id: 69,
        term: "ぷ",
        definition: "pu",
        syllableGroup: "u",
    },
    {
        id: 70,
        term: "ぺ",
        definition: "pe",
        syllableGroup: "e",
    },
    {
        id: 71,
        term: "ぽ",
        definition: "po",
        syllableGroup: "o",
    },
    {
        id: 72,
        term: "きゃ",
        definition: "kya",
        syllableGroup: "ya",
    },
    {
        id: 73,
        term: "きゅ",
        definition: "kyu",
        syllableGroup: "yu",
    },
    {
        id: 74,
        term: "きょ",
        definition: "kyo",
        syllableGroup: "yo",
    },
    {
        id: 75,
        term: "ぎゃ",
        definition: "gya",
        syllableGroup: "ya",
    },
    {
        id: 76,
        term: "ぎゅ",
        definition: "gyu",
        syllableGroup: "yu",
    },
    {
        id: 77,
        term: "ぎょ",
        definition: "gyo",
        syllableGroup: "yo",
    },
    {
        id: 78,
        term: "しゃ",
        definition: "sha",
        syllableGroup: "ha",
    },
    {
        id: 79,
        term: "しゅ",
        definition: "shu",
        syllableGroup: "hu",
    },
    {
        id: 80,
        term: "しょ",
        definition: "sho",
        syllableGroup: "ho",
    },
    {
        id: 81,
        term: "じゃ",
        definition: "ja",
        syllableGroup: "a",
    },
    {
        id: 82,
        term: "じゅ",
        definition: "ju",
        syllableGroup: "u",
    },
    {
        id: 83,
        term: "じょ",
        definition: "jo",
        syllableGroup: "o",
    },
    {
        id: 84,
        term: "ちゃ",
        definition: "cha",
        syllableGroup: "ha",
    },
    {
        id: 85,
        term: "ちゅ",
        definition: "chu",
        syllableGroup: "hu",
    },
    {
        id: 86,
        term: "ちょ",
        definition: "cho",
        syllableGroup: "ho",
    },
    {
        id: 87,
        term: "にゃ",
        definition: "nya",
        syllableGroup: "ya",
    },
    {
        id: 88,
        term: "にゅ",
        definition: "nyu",
        syllableGroup: "yu",
    },
    {
        id: 89,
        term: "にょ",
        definition: "nyo",
        syllableGroup: "yo",
    },
    {
        id: 90,
        term: "ひゃ",
        definition: "hya",
        syllableGroup: "ya",
    },
    {
        id: 91,
        term: "ひゅ",
        definition: "hyu",
        syllableGroup: "yu",
    },
    {
        id: 92,
        term: "ひょ",
        definition: "hyo",
        syllableGroup: "yo",
    },
    {
        id: 93,
        term: "びゃ",
        definition: "bya",
        syllableGroup: "ya",
    },
    {
        id: 94,
        term: "びゅ",
        definition: "byu",
        syllableGroup: "yu",
    },
    {
        id: 95,
        term: "びょ",
        definition: "byo",
        syllableGroup: "yo",
    },
    {
        id: 96,
        term: "ぴゃ",
        definition: "pya",
        syllableGroup: "ya",
    },
    {
        id: 97,
        term: "ぴゅ",
        definition: "pyu",
        syllableGroup: "yu",
    },
    {
        id: 98,
        term: "ぴょ",
        definition: "pyo",
        syllableGroup: "yo",
    },
    {
        id: 99,
        term: "みゃ",
        definition: "mya",
        syllableGroup: "ya",
    },
    {
        id: 100,
        term: "みゅ",
        definition: "myu",
        syllableGroup: "yu",
    },
    {
        id: 101,
        term: "みょ",
        definition: "myo",
        syllableGroup: "yo",
    },
    {
        id: 102,
        term: "りゃ",
        definition: "rya",
        syllableGroup: "ya",
    },
    {
        id: 103,
        term: "りゅ",
        definition: "ryu",
        syllableGroup: "yu",
    },
    {
        id: 104,
        term: "りょ",
        definition: "ryo",
        syllableGroup: "yo",
    },
]);

export const katakana = Object.freeze([
    {
        id: 105,
        term: "ア",
        definition: "a",
        syllableGroup: "a",
    },
    {
        id: 106,
        term: "イ",
        definition: "i",
        syllableGroup: "i",
    },
    {
        id: 107,
        term: "ウ",
        definition: "u",
        syllableGroup: "u",
    },
    {
        id: 108,
        term: "エ",
        definition: "e",
        syllableGroup: "e",
    },
    {
        id: 109,
        term: "オ",
        definition: "o",
        syllableGroup: "o",
    },
    {
        id: 110,
        term: "カ",
        definition: "ka",
        syllableGroup: "a",
    },
    {
        id: 111,
        term: "キ",
        definition: "ki",
        syllableGroup: "i",
    },
    {
        id: 112,
        term: "ク",
        definition: "ku",
        syllableGroup: "u",
    },
    {
        id: 113,
        term: "ケ",
        definition: "ke",
        syllableGroup: "e",
    },
    {
        id: 114,
        term: "コ",
        definition: "ko",
        syllableGroup: "o",
    },
    {
        id: 115,
        term: "サ",
        definition: "sa",
        syllableGroup: "a",
    },
    {
        id: 116,
        term: "シ",
        definition: "shi",
        syllableGroup: "hi",
    },
    {
        id: 117,
        term: "ス",
        definition: "su",
        syllableGroup: "u",
    },
    {
        id: 118,
        term: "セ",
        definition: "se",
        syllableGroup: "e",
    },
    {
        id: 119,
        term: "ソ",
        definition: "so",
        syllableGroup: "o",
    },
    {
        id: 120,
        term: "タ",
        definition: "ta",
        syllableGroup: "a",
    },
    {
        id: 121,
        term: "チ",
        definition: "chi",
        syllableGroup: "hi",
    },
    {
        id: 122,
        term: "ツ",
        definition: "tsu",
        syllableGroup: "su",
    },
    {
        id: 123,
        term: "テ",
        definition: "te",
        syllableGroup: "e",
    },
    {
        id: 124,
        term: "ト",
        definition: "to",
        syllableGroup: "o",
    },
    {
        id: 125,
        term: "ナ",
        definition: "na",
        syllableGroup: "a",
    },
    {
        id: 126,
        term: "ニ",
        definition: "ni",
        syllableGroup: "i",
    },
    {
        id: 127,
        term: "ヌ",
        definition: "nu",
        syllableGroup: "u",
    },
    {
        id: 128,
        term: "ネ",
        definition: "ne",
        syllableGroup: "e",
    },
    {
        id: 129,
        term: "ノ",
        definition: "no",
        syllableGroup: "o",
    },
    {
        id: 130,
        term: "ハ",
        definition: "ha",
        syllableGroup: "a",
    },
    {
        id: 131,
        term: "ヒ",
        definition: "hi",
        syllableGroup: "i",
    },
    {
        id: 132,
        term: "フ",
        definition: "fu",
        syllableGroup: "u",
    },
    {
        id: 133,
        term: "ヘ",
        definition: "he",
        syllableGroup: "e",
    },
    {
        id: 134,
        term: "ホ",
        definition: "ho",
        syllableGroup: "o",
    },
    {
        id: 135,
        term: "マ",
        definition: "ma",
        syllableGroup: "a",
    },
    {
        id: 136,
        term: "ミ",
        definition: "mi",
        syllableGroup: "i",
    },
    {
        id: 137,
        term: "ム",
        definition: "mu",
        syllableGroup: "u",
    },
    {
        id: 138,
        term: "メ",
        definition: "me",
        syllableGroup: "e",
    },
    {
        id: 139,
        term: "モ",
        definition: "mo",
        syllableGroup: "o",
    },
    {
        id: 140,
        term: "ヤ",
        definition: "ya",
        syllableGroup: "a",
    },
    {
        id: 141,
        term: "ユ",
        definition: "yu",
        syllableGroup: "u",
    },
    {
        id: 142,
        term: "ヨ",
        definition: "yo",
        syllableGroup: "o",
    },
    {
        id: 143,
        term: "ラ",
        definition: "ra",
        syllableGroup: "a",
    },
    {
        id: 144,
        term: "リ",
        definition: "ri",
        syllableGroup: "i",
    },
    {
        id: 145,
        term: "ル",
        definition: "ru",
        syllableGroup: "u",
    },
    {
        id: 146,
        term: "レ",
        definition: "re",
        syllableGroup: "e",
    },
    {
        id: 147,
        term: "ロ",
        definition: "ro",
        syllableGroup: "o",
    },
    {
        id: 148,
        term: "ワ",
        definition: "wa",
        syllableGroup: "a",
    },
    {
        id: 149,
        term: "ヲ",
        definition: "wo",
        syllableGroup: "o",
    },
    {
        id: 150,
        term: "ン",
        definition: "n",
        syllableGroup: "n",
    },
    {
        id: 151,
        term: "ガ",
        definition: "ga",
        syllableGroup: "a",
    },
    {
        id: 152,
        term: "ギ",
        definition: "gi",
        syllableGroup: "i",
    },
    {
        id: 153,
        term: "グ",
        definition: "gu",
        syllableGroup: "u",
    },
    {
        id: 154,
        term: "ゲ",
        definition: "ge",
        syllableGroup: "e",
    },
    {
        id: 155,
        term: "ゴ",
        definition: "go",
        syllableGroup: "o",
    },
    {
        id: 156,
        term: "ザ",
        definition: "za",
        syllableGroup: "a",
    },
    {
        id: 157,
        term: "ジ",
        definition: "ji",
        syllableGroup: "i",
    },
    {
        id: 158,
        term: "ズ",
        definition: "zu",
        syllableGroup: "u",
    },
    {
        id: 159,
        term: "ゼ",
        definition: "ze",
        syllableGroup: "e",
    },
    {
        id: 160,
        term: "ゾ",
        definition: "zo",
        syllableGroup: "o",
    },
    {
        id: 161,
        term: "ダ",
        definition: "da",
        syllableGroup: "a",
    },
    {
        id: 162,
        term: "ヂ",
        definition: "ji",
        syllableGroup: "i",
    },
    {
        id: 163,
        term: "ヅ",
        definition: "zu",
        syllableGroup: "u",
    },
    {
        id: 164,
        term: "デ",
        definition: "de",
        syllableGroup: "e",
    },
    {
        id: 165,
        term: "ド",
        definition: "do",
        syllableGroup: "o",
    },
    {
        id: 166,
        term: "バ",
        definition: "ba",
        syllableGroup: "a",
    },
    {
        id: 167,
        term: "ビ",
        definition: "bi",
        syllableGroup: "i",
    },
    {
        id: 168,
        term: "ブ",
        definition: "bu",
        syllableGroup: "u",
    },
    {
        id: 169,
        term: "ベ",
        definition: "be",
        syllableGroup: "e",
    },
    {
        id: 170,
        term: "ボ",
        definition: "bo",
        syllableGroup: "o",
    },
    {
        id: 171,
        term: "パ",
        definition: "pa",
        syllableGroup: "a",
    },
    {
        id: 172,
        term: "ピ",
        definition: "pi",
        syllableGroup: "i",
    },
    {
        id: 173,
        term: "プ",
        definition: "pu",
        syllableGroup: "u",
    },
    {
        id: 174,
        term: "ペ",
        definition: "pe",
        syllableGroup: "e",
    },
    {
        id: 175,
        term: "ポ",
        definition: "po",
        syllableGroup: "o",
    },
    {
        id: 176,
        term: "キャ",
        definition: "kya",
        syllableGroup: "ya",
    },
    {
        id: 177,
        term: "キュ",
        definition: "kyu",
        syllableGroup: "yu",
    },
    {
        id: 178,
        term: "キョ",
        definition: "kyo",
        syllableGroup: "yo",
    },
    {
        id: 179,
        term: "ギャ",
        definition: "gya",
        syllableGroup: "ya",
    },
    {
        id: 180,
        term: "ギュ",
        definition: "gyu",
        syllableGroup: "yu",
    },
    {
        id: 181,
        term: "ギョ",
        definition: "gyo",
        syllableGroup: "yo",
    },
    {
        id: 182,
        term: "シャ",
        definition: "sha",
        syllableGroup: "ha",
    },
    {
        id: 183,
        term: "シュ",
        definition: "shu",
        syllableGroup: "hu",
    },
    {
        id: 184,
        term: "ショ",
        definition: "sho",
        syllableGroup: "ho",
    },
    {
        id: 185,
        term: "ジャ",
        definition: "ja",
        syllableGroup: "a",
    },
    {
        id: 186,
        term: "ジュ",
        definition: "ju",
        syllableGroup: "u",
    },
    {
        id: 187,
        term: "ジョ",
        definition: "jo",
        syllableGroup: "o",
    },
    {
        id: 188,
        term: "チャ",
        definition: "cha",
        syllableGroup: "ha",
    },
    {
        id: 189,
        term: "チュ",
        definition: "chu",
        syllableGroup: "hu",
    },
    {
        id: 190,
        term: "チョ",
        definition: "cho",
        syllableGroup: "ho",
    },
    {
        id: 191,
        term: "ニャ",
        definition: "nya",
        syllableGroup: "ya",
    },
    {
        id: 192,
        term: "ニュ",
        definition: "nyu",
        syllableGroup: "yu",
    },
    {
        id: 193,
        term: "ニョ",
        definition: "nyo",
        syllableGroup: "yo",
    },
    {
        id: 194,
        term: "ヒャ",
        definition: "hya",
        syllableGroup: "ya",
    },
    {
        id: 195,
        term: "ヒュ",
        definition: "hyu",
        syllableGroup: "yu",
    },
    {
        id: 196,
        term: "ヒョ",
        definition: "hyo",
        syllableGroup: "yo",
    },
    {
        id: 197,
        term: "ビャ",
        definition: "bya",
        syllableGroup: "ya",
    },
    {
        id: 198,
        term: "ビュ",
        definition: "byu",
        syllableGroup: "yu",
    },
    {
        id: 199,
        term: "ビョ",
        definition: "byo",
        syllableGroup: "yo",
    },
    {
        id: 200,
        term: "ピャ",
        definition: "pya",
        syllableGroup: "ya",
    },
    {
        id: 201,
        term: "ピュ",
        definition: "pyu",
        syllableGroup: "yu",
    },
    {
        id: 202,
        term: "ピョ",
        definition: "pyo",
        syllableGroup: "yo",
    },
    {
        id: 203,
        term: "ミャ",
        definition: "mya",
        syllableGroup: "ya",
    },
    {
        id: 204,
        term: "ミュ",
        definition: "myu",
        syllableGroup: "yu",
    },
    {
        id: 205,
        term: "ミョ",
        definition: "myo",
        syllableGroup: "yo",
    },
    {
        id: 206,
        term: "リャ",
        definition: "rya",
        syllableGroup: "ya",
    },
    {
        id: 207,
        term: "リュ",
        definition: "ryu",
        syllableGroup: "yu",
    },
    {
        id: 208,
        term: "リョ",
        definition: "ryo",
        syllableGroup: "yo",
    },
]);

export const alphabet = Object.freeze([...hiragana, ...katakana]);
export const hiraganaTerms = Object.freeze(hiragana.map((x) => x.term));
export const katakanaTerms = Object.freeze(katakana.map((x) => x.term));
export const kanas = Object.freeze([...hiraganaTerms, ...katakanaTerms]);
export const syllableGroups = Object.freeze(
    Array.from(new Set(alphabet.map((x) => x.syllableGroup))),
);
export const totalsSyllableGroup = Object.freeze(
    Object.entries(Object.groupBy(hiragana, (x) => x.syllableGroup)).reduce(
        (x, y) => ({ ...x, [y[0]]: y[1].length }),
        {},
    ),
);

export const methodsKeys = Object.freeze({
    typing: "typing",
    listening: "listening",
    drawing: "drawing",
});

export const scores = Object.freeze({
    [methodsKeys.typing]: Object.freeze({ max: 100 }),
    [methodsKeys.listening]: Object.freeze({ max: 50 }),
    [methodsKeys.drawing]: Object.freeze({ max: 200, min: 50 }),
});

export const defaults = alphabet.reduce(
    (a, b) => ({
        ...a,
        [b.term]: Object.values(methodsKeys).reduce(
            (x, y) => ({ ...x, [y]: 0 }),
            {},
        ),
    }),
    {},
);
